...
//
void(*updatePtr)(float); void(*drawPtr)(); void(*mouseEventPtr)(UINT msg);
int fldSize = 20;
...
void drawCM() {
	...
	//RectangleM(hdc, camOffX/ fldSize * fldSize, camOffY / fldSize * fldSize, camOffX / fldSize * fldSize + fldSize, camOffY / fldSize* fldSize + fldSize);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
	...
	static HWND EDMazeSizeX, EDMazeSizeY;
	std::string s;
	MazeObject obj;
	static int ran = 0;
	switch (msg) {
	case WM_CREATE:
		...
		CreateWindow("static", "Maze Tile", WS_CHILD | WS_VISIBLE | SS_LEFT | SS_ENDELLIPSIS, drawRect.right+r.right - r.left + 5,0, 130, 25, hWnd, 0, 0, 0);
		EDMazeSizeX = CreateWindow("edit", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, drawRect.right, 55, 50, 25, hWnd, (HMENU)3, 0, 0);
		GetWindowRect(EDMazeSizeX, &r);
		CreateWindow("static", "Maze Size X", WS_CHILD | WS_VISIBLE | SS_LEFT | SS_ENDELLIPSIS, drawRect.right + r.right - r.left +5, 55, 130, 25, hWnd, 0, 0, 0);
		EDMazeSizeY = CreateWindow("edit", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, drawRect.right, 85, 50, 25, hWnd, (HMENU)3, 0, 0);
		GetWindowRect(EDMazeSizeY, &r);
		CreateWindow("static", "Maze Size Y", WS_CHILD | WS_VISIBLE | SS_LEFT | SS_ENDELLIPSIS, drawRect.right + r.right - r.left + 5, 85, 130, 25, hWnd, 0, 0, 0);
		CreateWindow("button", "SetSize", WS_CHILD | WS_VISIBLE | WS_BORDER, drawRect.right, 105, 150, 25, hWnd, (HMENU)0, 0, 0);

		CreateWindow("button", "maze size", WS_CHILD | WS_VISIBLE | WS_BORDER| BS_GROUPBOX, drawRect.right, 35, 180, 130-30, hWnd, 0, 0, 0);
